...
class AopCertification {
     ...
     */
    static function isTrusted($alipayCert, $rootCert)
    {
        $alipayCerts = AopCertification::readPemCertChain($alipayCert);
        $rootCerts = AopCertification::readPemCertChain($rootCert);
        if (AopCertification::verifyCertChain($alipayCerts, $rootCerts)) {
            return AopCertification::verifySignature($alipayCert, $rootCert);
        } else {
            return false;
        }
    ...
    static function verifySignature($alipayCert, $rootCert)
    {
        ...
        $checkSign = AopCertification::isCertSigner($alipayCertArray[0] . "-----END CERTIFICATE-----", $alipayCertArray[1] . "-----END CERTIFICATE-----");
        if (!$checkSign) {
            $checkSign = AopCertification::isCertSigner($alipayCertArray[1] . "-----END CERTIFICATE-----", $alipayCertArray[0] . "-----END CERTIFICATE-----");
            if ($checkSign) {
                ...
                for ($i = 0; $i < $length; $i++) {
                    ...
                    if ($issuer == $subject) {
                        AopCertification::isCertSigner($alipayCertArray[0] . "-----END CERTIFICATE-----", $rootCertArray[$i] . $rootCertArray);
                        return $checkSign;
                    }
                }
            } else {
            ...
            for ($i = 0; $i < $length; $i++) {
                $subject = openssl_x509_parse($rootCertArray[$i] . "-----END CERTIFICATE-----")['subject'];
                if ($issuer == $subject) {
                    $checkSign = AopCertification::isCertSigner($alipayCertArray[1] . "-----END CERTIFICATE-----", $rootCertArray[$i] . "-----END CERTIFICATE-----");
                    return $checkSign;
                }
            }
            return $checkSign;
    ...
    static function readPemCertChain($cert)
    {
    ...
    static function verifyCert($prev, $rootCerts)
    {
        $nowTime = time();
     ...
     */
    static function verifyCertChain($alipayCerts, $rootCerts)
    {
        $sorted = AopCertification::sortByDn($alipayCerts);
        if (!$sorted) {
        ...
        $prev = $alipayCerts[0];
        $firstOK = AopCertification::verifyCert($prev, $rootCerts);
     ...
     * @param $certs 证书链
     */
    static function sortByDn(&$certs)
    {
        //是否包含自签名证书
        ...
        for ($i = 0; $i < count($certs); $i++) {
            if (AopCertification::isSelfSigned($certs[$i])) {
        ...
        $certChain = null;
        AopCertification::addressingUp($subjectMap, $certChain, $certs[0]);
        AopCertification::addressingDown($issuerMap, $certChain, $certs[0]);
     ...
     * @param $cert 目标证书
     */
    static function isSelfSigned($cert)
    {
     ...
     */
    static function addressingUp($subjectMap, &$certChain, $current)
    {
        $certChain[] = $current;
        if (AopCertification::isSelfSigned($current)) {
            return;
        }
        ...
        AopCertification::addressingUp($subjectMap, $certChain, $subjectMap[$issuerDN]);
    }
     ...
     */
    static function addressingDown($issuerMap, &$certChain, $current)
    {
        ...
        AopCertification::addressingDown($issuerMap, $certChain, $issuerMap[$subjectDN]);
    }
    ...
    static function isCertSigner($certPem = null, $caCertPem = null)
    {
        ...
        $certDer = AopCertification::pemToDer($certPem);
        if (!is_string($certDer)) {
            die('invalid certPem');
        }
        // Grab the encrypted signature from the der encoded cert.
        $encryptedSig = AopCertification::extractSignature($certDer);
        ...
        $origCert = AopCertification::stripSignerAsn($certDer);
        if ($origCert === false) {
        ...
        $oid = AopCertification::getSignatureAlgorithmOid($decryptedSig);
        if ($oid === false) {
        ...
        $decryptedHash = AopCertification::getSignatureHash($decryptedSig);
